...
\begin{enumerate}
    ...
    \item (*) $\exists x~(P(x) \to \forall y~P(y))$
    \item (*) $\forall x~R(x,y) \to R(y,x), ~~ \forall xyz~R(x,y) \land R(y,z) \to R(x,z), ~~ \forall x \exists y~R(x,y) \vdash \forall x~R(x,x)$ \\ Подсказка: так как $\forall x \exists y~R(x,y)$, то для этого $y$ также верно $R(y,x)$, а из $R(x,y)$ и $R(y,x)$ заключаем $R(x,x)$.
\end{enumerate}
