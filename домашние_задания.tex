...
\begin{enumerate}
    ...
    \item $((p \lor q) \land (q \lor r)) \to (p \lor r)$
    ...
    \item \label{rules} Для каждой операции $\mathit{op}$ из следующего списка укажите, как в деревьях истинности разбирать уравнения видов $A~\mathit{op}~B = 1$ и $A~\mathit{op}~B = 0$: стрелка Пирса $\downarrow$, штрих Шеффера $\mid$, равносильность $\leftrightarrow$, сумма по модулю 2 $\oplus$ (она же исключающее или). Если какие-то из них незнакомы и не получится найти определение, напишите.
...
Можно использовать доказанные на семинаре секвенции как допустимые правила.
Для равносильностей требуется доказать оба направления.
\begin{enumerate}
...
\section{Истинность и формализация в логике предикатов}
\begin{enumerate}
    \item В модели с универсумом $\{a,b,c\}$ предикаты заданы таблицей:
    \[
        \begin{array}{c|ccc}
            x & a & b & c \\ \hline
            P(x) & 1 & 1 & 0 \\
        ...
        \end{array}
    \]
    Определите истинность формул; для незамкнутой формулы укажите все значения свободной переменной, при которых она истинна. Обоснуйте ответы.
    \begin{enumerate}
        \item $\forall x\,(P(x)\to Q(x))$;
        \item $\exists x\,(P(x)\land Q(x))$;
        \item $P(x)\to\forall y\,(Q(y)\to P(y))$.
    \end{enumerate}
    \item В модели с универсумом $\{a,b,c\}$ бинарный предикат $R$ задан таблицей (строки соответствуют первому аргументу, столбцы --- второму):
    \[
        \begin{array}{c|ccc}
            R & a & b & c \\ \hline
            a & 0 & 1 & 0 \\
        ...
        \end{array}
    \]
    Определите истинность формул и обоснуйте ответы:
    \begin{enumerate}
        \item $\forall x\,\exists y\,R(x,y)$;
        \item $\exists y\,\forall x\,R(x,y)$;
        \item $\exists x\,\exists y\,(R(x,y)\land R(y,x))$.
    \end{enumerate}
    \item[] Следующие утверждения переведите на язык логики предикатов. Укажите универсум (или сорта) и смысл используемых символов. Формализуйте утверждение независимо от того, истинно ли оно.
    ...
    \item Все слоны больше всех собак.
    \item Кошки бывают только белые и серые.
    \item Некоторые квадратные уравнения с действительными коэффициентами не имеют действительных решений (универсум "--- $\mathbb{R}$; используйте арифметические операции и равенство).
    \item Точки $A, B, C$ являются вершинами равнобедренного треугольника (используйте расстояние и предикат коллинеарности; исключите вырожденные треугольники.).
    \item У каждого положительного действительного числа есть ровно один положительный квадратный корень (универсум "--- $\mathbb{R}$; используйте арифметические операции и равенство, но не операцию взятия корня, так как она не везде определена).
    \item Есть сколько угодно большие простые числа (используйте формализацию простоты из семинара).
    \item Функция $f: \mathbb{R} \to \mathbb{R}$ непрерывна в точке $a$ (используйте определение предела через $\varepsilon$ и $\delta$).
    \itemhard Последовательность действительных чисел $a_0, a_1, \ldots$ имеет более одной предельной точки. Напоминание: $b$ --- предельная точка последовательности, если в любой её окрестности есть члены последовательности со сколь угодно большими индексами.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $\neg \exists x~(P(x) \to Q(x)) \vdash (\exists x~P(x)) \land (\forall x~Q(x))$
...
\begin{enumerate}
    ...
    \itemhard \label{h52} $\forall x \forall y~(R(x,y) \to R(y,x)),~~\forall x \forall y \forall z~(R(x,y) \land R(y,z) \to R(x,z)),~~\forall x \exists y~R(x,y) \vdash \forall x~R(x,x)$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Пусть $A,B,C$ множества, $f: A \to B, g: B \to C$ функции. Докажите:
        ...
        \end{enumerate}
    \item Пусть $A,B,C,D$ множества, $f: A \to C, g: B \to D$ вложения.
        \begin{enumerate}
            \item Если $A \cap B = C \cap D = \varnothing$, найдите вложение $h: A \cup B \to C \cup D$. Что изменится без этого дополнительного условия?
