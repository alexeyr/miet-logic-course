...
Перевести на язык логики предикатов. Укажите универсум (или сорта) и смысл используемых символов. Формализуйте утверждение независимо от того, истинно ли оно.
\begin{enumerate}
    ...
    \item Среднее арифметическое любых двух неотрицательных действительных чисел больше их среднего геометрического (не используйте операции деления и извлечения корня, так как они не везде определены).
    \item Функция $f: \mathbb{C} \to \mathbb{C}$ принимает в том числе такие комплексные значения, которые не являются действительными.
