...
\begin{itemize}
    ...
    \pause
    \item \emph{Ветвь} "--- путь от корня до листа. Её формулы должны выполняться одновременно.
\end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Можно доказать больше: если $A$ тождественно истинна, то любое законченное дерево истинности для $A=0$ закрыто.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Для $\beta$: $\beta \in \mathcal{B} \Rightarrow \beta_1 \in \mathcal{B} \lor \beta_2 \in \mathcal{B}$ (т.к. $\beta$ разобрана) $\Rightarrow$ на $\sigma$ выполняется $\beta_1$ или $\beta_2$ (по предположению индукции) $\Rightarrow$ выполняется и $\beta$.
        \pause
