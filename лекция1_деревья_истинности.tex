...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \pause
        \item В ЛВ оба предложения считаем \emph{атомарными}: каждое представляется одной пропозициональной переменной.
        \item В логике предикатов их будем разбирать подробнее.
        \pause
...
\onslide<6->{
    \item Пусть $p \land q \to q \land p = 0$.
\item Тогда $p \land q = 1$ и $q \land p = 0$.
\item Тогда $p = 1$ и $q = 1$.
\item Отсюда $q = 0$ или $p = 0$. Но оба невозможны!
\item Значит, $p \land q \to q \land p = 0$ невозможно.
\item $p \land q \to q \land p$ тождественно истинно!
}
