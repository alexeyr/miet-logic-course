...
\begin{itemize}
    \item Начнём с примера. Нужно доказать $p \land q \to q \land p$:
    \item[] 
\only<1>{
    \begin{tableau}{
    }
...
\begin{itemize}
    \item Идея в поиске контрпримера для утверждения, которое хотим доказать.
    \item Для доказательства формулы $A$ мы начинаем с уравнения $A=0$.
    \item Чтобы проверить, следует ли $B$ из посылок $A_1,\, A_2,\,\ldots$, начинаем с \pause $A_1=1,\, A_2=1,\, \ldots,\, B=0$ (пишем их в столбик).
