...
\begin{itemize}
    \item \emph{Деревья истинности} "--- способ проверить, является ли формула тождественно истинной.
    \pause
    \item Идея: ищем контрпример "--- оценку, на которой формула ложна.
    \pause
    \item Если контрпример нашёлся, формула не тождественно истинна. 
    \item Если контрпримера нет "--- тождественно истинна.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Значит, у исходного утверждения нет контрпримера и оно доказано.
    \pause
    \item Если какая-то открытая ветвь \emph{закончена} (все её уравнения разобраны), то мы нашли контрпример к исходной формуле и она \emph{не} тождественно истинна.
    \pause
    \item Дерево \emph{закончено}, если оно закрыто или в нём есть законченная открытая ветвь: ответ уже получен, остальные ветви можно не разбирать.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Идея в поиске контрпримера для утверждения, которое хотим проверить.
